...
def test_process_pdf_with_fallback_docling_success(mock_unstructured, mock_docling):
    """Test that docling success skips fallbacks when GPU is available."""
    mock_docling.return_value = True
    
    with patch('agent.node.ingestion.torch', create=True) as mock_torch:
...
@patch('agent.node.ingestion.parse_pdf_with_docling')
@patch('agent.node.ingestion.parse_pdf_with_unstructured')
def test_process_pdf_with_fallback_unstructured_success(mock_unstructured, mock_docling):
...
@patch('agent.node.ingestion.parse_pdf_with_docling')
@patch('agent.node.ingestion.parse_pdf_with_unstructured')
def test_process_pdf_with_fallback_docling_fails(mock_unstructured, mock_docling):
